...
class Register(AdminDashboard):
    def __init__(self, root):
        ...
        license_number = Label(self.root, text="License Number:")
        license_number.place(x=50, y=260)
        self.license_number_entry = Entry(self.root, width=25)
        self.license_number_entry.place(x=150, y=260)
    ...
    def register(self):
        try:
            ...
            query = """
                    INSERT INTO driver (name, email, contact, address, username, password, licenceid, status)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                """
            values = (self.name_entry.get(), self.email_entry.get(), self.contact_entry.get(), self.address_entry.get(), self.username_entry.get(), self.password_entry.get(), self.license_number_entry.get(), "Free")
            cursor.execute(query, values)
            self.clearWindow()
            messagebox.showinfo("Registration", "Driver Registration Success")
            
            # Going back to the login page after registration
            from AdminDashboard import AdminDashboard
